...
## 2026-06-16 - Rate Limit Bypass via Password Fallback in Token Verification

**Vulnerability:** The `verifyAdminToken` middleware checked active session tokens but fell back to accepting the static admin password as a Bearer token if the session was invalid. Because the failed authentication rate limiter (`adminRateLimitTracker`) was only mounted on the dedicated `/api/admin/verify` login route, an attacker could continuously brute-force the admin password by sending requests with guessed passwords in the Authorization header to other protected endpoints (like `/api/admin/maintenance-config`), completely bypassing the rate limiter.
**Learning:** Incomplete migration to session tokens or leaving alternate authentication paths unmonitored can inadvertently bypass security controls like rate limiting. If a rate limiter tracks failures on a specific endpoint, alternate endpoints must not allow the same authentication method.
**Prevention:** Remove fallback authentication mechanisms. When migrating to session tokens, ensure that protected endpoints strictly require valid session tokens and do not evaluate raw passwords directly.
